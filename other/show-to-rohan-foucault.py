...
write_to_file(f"{DAC}/out_voltage0_raw", str(0 * (1/DAC_VOLTAGE_SCALE)))
time.sleep(5)
#####
...
coarse_tune = PID(0.6, 0.15, 0.1, setpoint=PHASEDET_AVGVOLT)
coarse_tune.output_limits = (-2000, 2000)
fine_tune = PID(0.25, 0.60, 0.05, setpoint=PHASEDET_AVGVOLT)
...
while True:
    ...
    if abs(error) > LIMIT:
        ...
        print(f"PID_c: {PHASEDET_AVGVOLT:.3f}, {phase_voltage:.3f}, {correction * DAC_VOLTAGE_SCALE:.3f}, {phase_voltage - PHASEDET_AVGVOLT:.3f}")
        time.sleep(0.1)
        ...
        # write_to_file(f"{DAC}/out_voltage0_raw", str(correction * 0.9))
        write_to_file(f"{DAC}/out_voltage0_raw", str(correction * 1))
        
        # Print phase voltage and correction voltage
        print(f"PID_f: {PHASEDET_AVGVOLT:.3f}, {phase_voltage:.3f}, {correction * DAC_VOLTAGE_SCALE:.3f}, {phase_voltage - PHASEDET_AVGVOLT:.3f}")
        time.sleep(0.1 * finecounter)
        if abs(error) < 2 and finecounter < 5:
            if not finecounter > 5:
                finecounter+=1
